...
flags.DEFINE_integer(
    'read_parallelism', default=32,
    help=('The number of parallel interleaves for TFRecords'))
...
flags.DEFINE_integer(
    'map_tfrecord_decode_parallelism', default=64,
    help=(''))
...
flags.DEFINE_integer(
    ...
    help=('The number of parallel transposes'))

flags.DEFINE_integer(
    'shard_parallelism', default=tf.data.experimental.AUTOTUNE,
    help=(''))
...
def ssd_input_pipeline(params, file_pattern, is_training=False,
  ...
  if is_training:
    return ssd_input_ds.map(
        shard_train_batch,
        num_parallel_calls=FLAGS.shard_parallelism).repeat()
  else:
    return ssd_input_ds.map(
        shard_eval_batch,
        num_parallel_calls=FLAGS.shard_parallelism).repeat()
