...
test_dir_name="train_sweep_v2_resnet_0_PDL"
...
data_dir="/mnt/data/BigLearning/mkuchnik/datasets/ImageNet/ILSVRC12_TFRecord/train/"
global_opt="--data_dir=$data_dir --cache_records=True --benchmark_num_elements=1500000 --dataset_threadpool_size=32 --time_limit_s=72 --percentage_cached=0.0 --map_and_batch_fusion=True"
...
function run_heuristic() {
  test_name=$test_dir_name/heuristic_$heuristic_i
  mkdir -p $test_name
  pushd $test_name
  step_heuristic
  popd
...
function run_autotune() {
  test_name=$test_dir_name/autotune_$autotune_i
  mkdir -p $test_name
  pushd $test_name
  step_autotune
  popd
...
run_baseline
for heuristic_i in 0 1 2
do
  run_heuristic
done
for autotune_i in 0 1 2
do
  run_autotune
done
for plumber_i in 0
