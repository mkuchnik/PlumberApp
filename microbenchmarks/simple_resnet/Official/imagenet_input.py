...
def process_record_dataset(dataset,
  ...
  if is_training:
    ...
    if FLAGS.shuffle_size != shuffle_buffer:
        tf.print("Overriding shuffle buffer:"
                 " {}".format(FLAGS.shuffle_size))
    dataset = dataset.shuffle(buffer_size=FLAGS.shuffle_size)
  ...
  dataset = dataset.map(
      map_fn, num_parallel_calls=FLAGS.map_parse_parallelism)
...
def preprocess_parsed_example(image_buffer,
                              label,
                              bbox,
                              is_training,
                              dtype,
...
def parse_and_preprocess_record(raw_record,
                                is_training,
                                dtype,
                                num_classes,
                                one_hot=False):
  ...
  return preprocess_parsed_example(
      image_buffer=image_buffer,
      label=label,
      bbox=bbox,
      is_training=is_training,
...
def input_fn(is_training,
  ...
  dataset = dataset.interleave(
      ...
      cycle_length=FLAGS.read_parallelism,
      num_parallel_calls=FLAGS.read_parallelism)
  ...
  if is_training and FLAGS.cache_decoded_image:
    dataset = dataset.map(
        ...
        num_parallel_calls=FLAGS.map_parse_protobuf_parallelism)
  if dataset_cache:
...
def _decode_crop_and_flip(image_buffer, bbox, num_channels):
  ...
  shape = (
      ...
      if decoded else tf.image.extract_jpeg_shape(image_buffer))
...
# Alias
get_input_fn = input_fn


def main(_):
    data_dir = FLAGS.data_dir
    batch_size = FLAGS.batch_size
    dataset = input_fn(is_training=True,
                       data_dir=data_dir,
                       batch_size=batch_size)
    run_dataset(dataset)
