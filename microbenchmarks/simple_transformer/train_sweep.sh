...
test_dir_name="train_sweep_0_transformer_plain_batch"
...
train_data_path="/zpool1/Datasets/Official/MLPerf/wmt32k/translate_ende_official_tfrecord_packed/translate*-train-*"

global_opt="--benchmark_num_elements=150000000 --dataset_threadpool_size=16 --time_limit_s=62 --map_and_batch_fusion=True"
...
function step_heuristic {
  ...
  python3 $benchmark_path \
    ...
    --map_2_parallelism=16 \
    --map_3_parallelism=16 \
    --use_cache=True \
    --train_data_path=${train_data_path} \
    ${global_opt} 2>&1 | tee ${name}_log.txt
  cp stats.pb $name.pb
...
for plumber_i in 1 2 3 4 5
do
  run_plumber_rewrites
done
for random_i in 3 4 5
do
  run_random_rewrites
done
