...
flags.DEFINE_string('data_dir', "/zpool1/Datasets/Official/MLPerf/COCO", 'The directory for TFRecords')
flags.DEFINE_integer('batch_size', 4, 'The batch size to use')
...
flags.DEFINE_integer(
    'read_parallelism', default=None,
    help=(''))
flags.DEFINE_integer(
    'map_parse_parallelism', default=None,
    help=(''))
flags.DEFINE_integer(
    'map_transform_images_parallelism', default=None,
    help=(''))
flags.DEFINE_bool(
    'map_and_batch_fusion', default=True,
    help=('tf.data options'))
flags.DEFINE_integer(
    'inner_map_parallelism', default=None,
    help=(''))
flags.DEFINE_bool(
    'cache_records', default=True,
    help=(''))
flags.DEFINE_integer(
  ...
  def crop_gt_masks(self, gt_mask_size):
    ...
    with tf.control_dependencies([assert_length]):
      cropped_gt_masks = tf.image.crop_and_resize(
          ...
          #box_ind=tf.range(num_masks, dtype=tf.int32), # Note(mkuchnik): deprecated
...
class InputReader(object):
  ...
  def __call__(self, params, num_examples=0):
    ...
    concurrent_files = FLAGS.read_parallelism
    ...
    if shuffle_data:
      ...
      if FLAGS.cache_records:
          dataset = \
          dataset.take(500).cache().shuffle(params['shuffle_buffer_size'],
    ...
    dataset = dataset.map(_dataset_parser,
                          num_parallel_calls=FLAGS.map_parse_parallelism)
    ...
    if (self._mode == tf.estimator.ModeKeys.PREDICT and
      ...
      dataset_padding_hor = dataset_padding.filter(horizontal_image).map(
          _mark_is_padding, FLAGS.inner_map_parallelism).take(pad_size)
      ...
      if self._distributed_eval:
        dataset = dataset.map(_mark_is_not_padding,
                              FLAGS.inner_map_parallelism).take(
            int(
                math.ceil(params['eval_samples'] /
                          params['dataset_num_shards'])))
      else:
        dataset = dataset.map(_mark_is_not_padding,
                              FLAGS.inner_map_parallelism
                              ).take(params['eval_samples'])
    ...
    dataset = dataset.map(
        ...
        num_parallel_calls=FLAGS.map_transform_images_parallelism)
...
def get_loader_fn(data_dir: str):
    import os
    training_file_pattern = os.path.join(data_dir, "train-*.tfrecord")
    use_fake_data = False
...
def get_dataset(data_dir: str):
    dataset_fn = get_loader_fn(data_dir)
    ...
    params = dict(
        ...
        val_json_file=None,
        num_cores_per_replica=int(4), # TODO(mkuchnik): Change?
    ) 
    params["batch_size"] = FLAGS.batch_size
    params["replicas_per_host"] = 1
...
def run_dataset(dataset):
    if FLAGS.is_profile:
        print("Profling")
        with tf.profiler.experimental.Profile(FLAGS.logdir):
