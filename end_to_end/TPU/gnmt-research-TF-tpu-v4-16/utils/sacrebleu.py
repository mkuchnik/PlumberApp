...
def smart_open(file, mode='rt', encoding='utf-8'):
    ...
    return io.open(file, mode=mode, encoding=encoding, newline="\n")
...
def download_test_set(test_set, langpair=None):
    ...
    for dataset, expected_md5 in zip(DATASETS[test_set]['data'], expected_checksums):
        ...
        if not os.path.exists(tarball) or os.path.getsize(tarball) == 0:
            ...
            try:
                f = urllib.request.urlopen(dataset)
...
def corpus_bleu(sys_stream,
    ...
    for lines in zip_longest(*fhs):
        ...
        tokenized = [TOKENIZERS[tokenize](x.rstrip()) for x in lines]
        output, refs = tokenized[0], tokenized[1:]
...
def main():
    ...
    if args.test_set:
        refs = download_test_set(args.test_set, args.langpair)[1:]
        if len(refs) == 0:
