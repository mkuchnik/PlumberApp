...
def add_argument(parser):
  parser.add_argument(
      '--input_pipeline_read_parallelism', default=100, type=int,
      help=(''))

  parser.add_argument(
      '--input_pipeline_map_0_parallelism', default=64, type=int,
      help=(''))

  parser.add_argument(
      '--input_pipeline_map_1_parallelism', default=64, type=int,
      help=(''))

  parser.add_argument(
      '--input_pipeline_default_prefetching', default=100, type=int,
      help=(''))

  parser.add_argument(
    '--input_pipeline_cache', default=False, type=bool,
...
def get_preprocessed_iterator(dataset_file,
                              ...
                              my_custom_iterator_flags=None,
                              ):
  """Get the dataset iterator from preprocessed data."""
  assert my_custom_iterator_flags

  FLAGS = my_custom_iterator_flags
  def READ_PARALLELISM():
      ret = FLAGS.input_pipeline_read_parallelism
      tf.logging.info("Read parallelism={}".format(ret))
      #ret = 1
      return ret
  
  def MAP0_PARALLELISM():
      ret = FLAGS.input_pipeline_map_0_parallelism
      tf.logging.info("Map0 parallelism={}".format(ret))
      #ret = 1
      return ret
  
  def MAP1_PARALLELISM():
      ret = FLAGS.input_pipeline_map_1_parallelism
      tf.logging.info("Map1 parallelism={}".format(ret))
      #ret = 1
      return ret
  
  def DEFAULT_PREFETCHING():
      ret = FLAGS.input_pipeline_default_prefetching
      tf.logging.info("Prefetching={}".format(ret))
      #ret = 1
      return ret
  
  def PIPELINE_CACHE():
      ret = FLAGS.input_pipeline_cache
      tf.logging.info("Caching={}".format(ret))
      #ret = False
      return ret
  ...
  #dataset = dataset.apply(
  #    tf.data.experimental.parallel_interleave(
  #        fetch_dataset, cycle_length=READ_PARALLELISM(), sloppy=True))

  dataset = dataset.interleave(
          fetch_dataset, READ_PARALLELISM(), 1, READ_PARALLELISM())
  ...
  if PIPELINE_CACHE():
    #assert False, "Caching isn't enabled"
    src_tgt_dataset = dataset.map(
        _parse, num_parallel_calls=MAP0_PARALLELISM()).cache()
  else:
    src_tgt_dataset = dataset.map(
        _parse, num_parallel_calls=MAP0_PARALLELISM())
  ...
  batched_dataset = batched_dataset.map(
      ...
      num_parallel_calls=MAP1_PARALLELISM()).prefetch(DEFAULT_PREFETCHING())
  # TODO remove
  #batches_dataset = batched_dataset.take(1).cache().repeat()
  return batched_dataset
