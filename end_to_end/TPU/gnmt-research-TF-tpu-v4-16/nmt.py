...
def create_hparams(flags):
  ...
  train_dir = flags.local_data_dir if flags.local_data_dir else flags.data_dir
  ...
  def check_steps_epochs_compatibility():
    """The math done here is possible to have rounding errors, so throw early.

    For example, steps % iterations == 0, but it may not
    Example is 33616%6723 = 1, so this is an error
    """
    _num_train_steps=int(flags.num_examples_per_epoch / flags.batch_size *
                         flags.max_train_epochs)
    _steps_per_epoch = int(flags.num_examples_per_epoch/flags.batch_size)
    iterations = _steps_per_epoch
    is_compat = (_num_train_steps % iterations == 0)
    if not is_compat:
        raise ValueError(
                "num_steps = {} ({}), but iterations is {} ({}) and is not divisible ({})".format(
                    _num_train_steps,
                    "{} / {} * {}".format(flags.num_examples_per_epoch, flags.batch_size, flags.max_train_epochs),
                    iterations,
                    "{} / {}".format(flags.num_examples_per_epoch, flags.batch_size),
                    _num_train_steps % iterations,
                    ))

  check_steps_epochs_compatibility()
  ...
  return compat_hparam.HParams(
      ...
      src=flags.src,
      tgt=flags.tgt,
      #train_prefix=train_dir + flags.train_prefix,
...
def run_main(flags, default_hparams, estimator_fn):
  ...
  hparams.my_custom_iterator_flags = flags
...
if __name__ == "__main__":
  ...
  iterator_utils.add_argument(nmt_parser)
