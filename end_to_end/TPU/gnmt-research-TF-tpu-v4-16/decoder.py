...
#from tensorflow.compat.v1.contrib import framework as contrib_framework
...
def _create_zero_outputs(size, dtype, batch_size):
  ...
  def _create(s, d):
    return tf.zeros([batch_size, s], d)

  return contrib_framework.nest.map_structure(_create, size, dtype)
...
def dynamic_decode(decoder,
  ...
  with tf.variable_scope(scope, "decoder") as varscope:
    ...
    def _create_ta(s, d):
      return tf.zeros([maximum_iterations, decoder.batch_size, s], dtype=d)
    ...
    def body(time, outputs_ta, state, inputs, finished, sequence_lengths):
      ...
      contrib_framework.nest.assert_same_structure(outputs_ta, next_outputs)
      contrib_framework.nest.assert_same_structure(inputs, next_inputs)

      # Zero out output values past finish
      if impute_finished:
        emit = contrib_framework.nest.map_structure(
            lambda out, zero: tf.where(finished, zero, out), next_outputs,
            zero_outputs)
      else:
        emit = next_outputs
      ...
      outputs_ta = contrib_framework.nest.map_structure(
          lambda ta, out: inplace_ops.alias_inplace_update(ta, time, out),
          outputs_ta, emit)
