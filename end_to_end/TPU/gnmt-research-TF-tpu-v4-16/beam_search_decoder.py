...
contrib_util.constant_value = tf.get_static_value 
...
def tile_batch(t, multiplier, name=None):
  ...
  Raises:
    ...
    the rank is < 1.
  """
  flat_t = contrib_framework.nest.flatten(t)
  with tf.name_scope(name, "tile_batch", flat_t + [multiplier]):
    return contrib_framework.nest.map_structure(
...
class BeamSearchDecoder(decoder.Decoder):
  ...
  def __init__(self,
    ...
    self._batch_size = get_dim_as_int(start_tokens.shape[0])
    self._beam_width = beam_width
    self._max_tgt = max_tgt
    self._length_penalty_weight = length_penalty_weight
    self._coverage_penalty_weight = coverage_penalty_weight
    self._initial_cell_state = contrib_framework.nest.map_structure(
  ...
  def output_size(self):
    ...
    return BeamSearchDecoderOutput(
        scores=self._beam_width,
        predicted_ids=self._beam_width,
        parent_ids=self._beam_width)
  ...
  def output_dtype(self):
    ...
    return BeamSearchDecoderOutput(
        scores=contrib_framework.nest.map_structure(
  ...
  def _merge_batch_beams(self, t, s=None):
    ...
    if isinstance(s, tf.Tensor):
      s = contrib_util.constant_value(s)
  ...
  def _split_batch_beams(self, t, s=None):
    ...
    if t.shape.ndims < 1:
      return t
    if isinstance(s, tf.Tensor):
      s = tf.TensorShape(contrib_util.constant_value(s))
    else:
      s = tf.TensorShape(s)
    t_shape = tf.shape(t)
    reshaped_t = tf.reshape(
  ...
  def step(self, time, inputs, state, name=None):
    ...
    with tf.name_scope(name, "BeamSearchDecoderStep", (time, inputs, state)):
      ...
      inputs = contrib_framework.nest.map_structure(
          lambda inp: self._merge_batch_beams(inp, s=inp.shape[2:]), inputs)
      cell_state = contrib_framework.nest.map_structure(self._merge_batch_beams,
                                                        cell_state)
      next_cell_state, cell_outputs = self._cell(inputs, cell_state)
      cell_outputs = contrib_framework.nest.map_structure(
...
def _beam_search_step(time, logits, next_cell_state, beam_state, batch_size,
  ...
  vocab_size = get_dim_as_int(logits.shape[-1]) or tf.shape(logits)[-1]
...
def _length_penalty(sequence_lengths, penalty_factor, dtype):
  ...
  static_penalty = contrib_util.constant_value(penalty_factor)
