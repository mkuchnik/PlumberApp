...
def get_raw_dataset(dataset_builder: tfds.core.DatasetBuilder,
  ...
  tfds_config = tfds.ReadConfig(
          ...
          interleave_cycle_length=AUTOTUNE,
...
def pack_dataset(dataset: tf.data.Dataset,
  ...
  # trim to lengtemap/batch
  # NOTE(mkuchnik): Canonical Map 3
  #dataset = dataset.map(
  #    lambda x: {k: x[k][:key2length[k]] for k in keys},
  #    num_parallel_calls=AUTOTUNE)
  parallel_map_3_parallelism = AUTOTUNE
  dataset = dataset.map(
      lambda x: {k: x[k][:key2length[k]] for k in keys},
      num_parallel_calls=parallel_map_3_parallelism)
  ...
  # NOTE(mkuchnik): Map/batch
  map_and_batch_parallelism = AUTOTUNE
  #return dataset.map(my_fn, num_parallel_calls=AUTOTUNE)
  return dataset.map(my_fn, num_parallel_calls=map_and_batch_parallelism)
...
def _pack_with_tf_ops(dataset: tf.data.Dataset, keys: List[str],
  ...
  # Canonical ParallelMapDataset 4
  #dataset = dataset.map(map_fn, num_parallel_calls=AUTOTUNE)
  parallel_map_4_parallelism = AUTOTUNE
  dataset = dataset.map(map_fn, num_parallel_calls=parallel_map_4_parallelism)
  # NOTE(mkuchnik): map -> unbatch -> parallel_map_4
  # unbatch implicitly add mapdataset for tensor normalization
