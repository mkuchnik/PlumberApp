...
try:
    ...
    contrib_training = None
    import efficientnet_preprocessing.compat.hparam
    HParams = efficientnet_preprocessing.compat.hparam.HParams
...
def rotate_image(image, radians):
    """Translate a wrapped image by radians"""
    if contrib_image:
        return contrib_image.rotate(image, radians)
    elif tfa:
        return tfa.image.rotate(image, radians)
    raise NotImplemented("Neither tf.contrib nor Tensorflow addons are available")

def translate_image(image, tx_ty):
    """Translate a wrapped image by tx and ty (passed as vector/list
    of length 2)"""
    if contrib_image:
        return contrib_image.translate(image, tx_ty)
    elif tfa:
        return tfa.image.translate(image, tx_ty)
    raise NotImplemented("Neither tf.contrib nor Tensorflow addons are available")

def transform_image(image, transforms):
    """Transforms a wrapped image by projective transform matrix
    (passed as vector/list of length 8)"""
    if contrib_image:
        contrib_image.transform(image, transforms)
    elif tfa:
        return tfa.image.transform(image, transforms)
    raise NotImplemented("Neither tf.contrib nor Tensorflow addons are available")
...
def rotate(image, degrees, replace):
  ...
  # of the function.
  image = rotate_image(wrap(image), radians)
  return unwrap(image, replace)


def translate_x(image, pixels, replace):
  """Equivalent of PIL Translate in X dimension."""
  image = translate_image(wrap(image), [-pixels, 0])
  return unwrap(image, replace)


def translate_y(image, pixels, replace):
  """Equivalent of PIL Translate in Y dimension."""
  image = translate_image(wrap(image), [0, -pixels])
  return unwrap(image, replace)
