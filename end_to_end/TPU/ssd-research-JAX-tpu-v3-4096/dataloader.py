...
class SSDInputReader(object):
  ...
  def _decode(self, data):
    # Note(mkuchnik): This does both image decoding as well as TFRecord
  ...
  def __call__(self, params):
    ...
    dataset = dataset.interleave(tf.data.TFRecordDataset,
                                 FLAGS.read_parallelism or None, 1,
                                 FLAGS.read_parallelism or None)
    ...
    dataset = dataset.map(self._decode,
                          num_parallel_calls=FLAGS.map_tfrecord_decode_parallelism or None)
    ...
    if self._is_training:
      ...
      # TODO(mkuchnik): We disable ad-hoc caches
    ...
    dataset = dataset.map(self._parse_example,
                          num_parallel_calls=FLAGS.map_parse_parallelism or None)
    ...
    if self._params['conv0_space_to_depth']:
      ...
      dataset = dataset.map(_space_to_depth_fn, num_parallel_calls=FLAGS.map_image_postprocessing_parallelism or None)
      ...
      dataset = dataset.map(_transpose_dataset,
                            num_parallel_calls=FLAGS.map_image_transpose_postprocessing_parallelism or None)

    if FLAGS.prefetch_amount:
        tf.print("Adding prefetch of {}".format(FLAGS.prefetch_amount))
        dataset = dataset.prefetch(FLAGS.prefetch_amount)
    else:
        tf.print("Not adding prefetch")
    options = tf.data.Options()
    options.experimental_threading.max_intra_op_parallelism = 1
    options.experimental_threading.private_threadpool_size = 96
    # NOTE(mkuchnik): override self._params['dataset_threadpool_size']
