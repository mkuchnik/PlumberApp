...
def non_max_suppression_padded(scores, boxes, max_output_size, iou_threshold):
  ...
  #num_boxes_float = np.float32(num_boxes)
  #pad = int(jnp.ceil(float(num_boxes) / _NMS_TILE_SIZE)
  #         ) * _NMS_TILE_SIZE - num_boxes
  pad = (np.ceil(float(num_boxes) / float(_NMS_TILE_SIZE)).astype(int)
           ) * _NMS_TILE_SIZE - num_boxes
  # TODO(mkuchnik): depreacated code
  #pad_ratio = jnp.ceil(num_boxes_float / _NMS_TILE_SIZE).astype(int)
  #pad =  pad_ratio * _NMS_TILE_SIZE - num_boxes
